...
import torch
from utilities.common_loss import My_KL_loss
from utilities.common_loss import My_logit_ML_loss
from utilities.common_tools import gen_adj
from model.GAT import GAT

class MMSFI_C_Model(nn.Module):

    def __init__(self, view_blocks, comm_feature_num, label_num, model_args=None):
        super(MMSFI_C_Model, self).__init__()
        ...
        self.gat = GAT(comm_feature_num, comm_feature_num,  dropout=0.1, alpha=0.1, nheads=view_count)
    ...
    def forward(self, x, is_training=True, labels = None):
        ...
        comm_feature_adj = gen_adj(comm_feature)
        comm_feature, head = self.gat(comm_feature,comm_feature_adj)
    ...
    def get_config_optim(self, lr, lrp):
        return [
                {'params': self.fc_comm_extract.parameters(), 'lr': lr * lrp},
                {'params': self.fc_comm_predictor.parameters(), 'lr': lr},
                {'params': self.fc_predictor.parameters(), 'lr': lr},
                ]
